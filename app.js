...
function useLocalStore(key, initial) {
  const [val, setVal] = useState(() => {
    ...
    catch (e2) { return initial; }
  });
  const save = (v) => { setVal(v); try { localStorage.setItem(key, JSON.stringify(v)); } catch (e3) {} };
  return [val, save];
}
...
async function claudeChat(systemPrompt, userMessage, history = [], maxTokens = 1000) {
  try {
    ...
    return "__ERR__:" + (_optionalChain([e, 'optionalAccess', _ => _.message]) || "network error");
  }
}
...
function exportIEPReport(analysis) {
  ...
  ${_optionalChain([analysis, 'access', _2 => _2.redFlags, 'optionalAccess', _3 => _3.length])?`<div class="section"><h2 class="red">🚨 Red Flags</h2>${analysis.redFlags.map(f=>`<div class="item red">• ${f}</div>`).join("")}</div>`:""}
  ${_optionalChain([analysis, 'access', _4 => _4.legalConcerns, 'optionalAccess', _5 => _5.length])?`<div class="section"><h2 class="gold">⚖️ Legal Concerns</h2>${analysis.legalConcerns.map(l=>`<div class="item">§ ${l}</div>`).join("")}</div>`:""}
...
const GradText = ({ children, a=C.teal, b=C.lavender }) => (
  React.createElement('span', { style: { background:`linear-gradient(135deg,${a},${b})`,
    WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text" },}, children)
);
const Pill = ({ children, color=C.teal }) => (
  React.createElement('span', { style: { display:"inline-block", padding:"4px 14px", borderRadius:999, fontSize:11,
    fontWeight:700, letterSpacing:"0.08em", background:`${color}1A`, color,
    border:`1.5px solid ${color}44`, textTransform:"uppercase" },}, children)
);
const Btn = ({ children, onClick, variant="primary", size="md", style:s={}, disabled }) => {
  ...
  return React.createElement('button', { onClick: onClick, disabled: disabled, style: { ...sz[size], ...vr[variant], borderRadius:12, fontFamily:font, fontWeight:700, cursor:disabled?"not-allowed":"pointer", opacity:disabled?0.5:1, transition:"all 0.2s", ...s },}, children);
};
function Card({ children, style={}, glow, onClick }) {
  return React.createElement('div', { onClick: onClick, style: { background:C.cardBg, borderRadius:20, padding:28,
    boxShadow:glow?`0 8px 40px ${C.teal}22,0 2px 12px rgba(0,0,0,0.06)`:"0 2px 16px rgba(0,0,0,0.06)",
    border:`1px solid ${C.border}`, cursor:onClick?"pointer":"default", ...style },}, children);
}
const Tag = ({ c=C.teal, children }) => (
  React.createElement('span', { style: { background:`${c}18`, color:c, border:`1px solid ${c}33`, borderRadius:6, padding:"3px 10px", fontSize:11, fontWeight:700 },}, children)
);
const Input = ({ value, onChange, placeholder, type="text", style:s={} }) => (
  React.createElement('input', { value: value, onChange: onChange, placeholder: placeholder, type: type,
...
function Nav({ active, setActive, user, setUser, lang, setLang, t }) {
  ...
  return (
    React.createElement('nav', { style: { position:"fixed", top:0, left:0, right:0, zIndex:200,
      background:"rgba(253,248,242,0.96)", backdropFilter:"blur(20px)",
      borderBottom:`1px solid ${C.border}`, height:64,
      display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 20px" },}
      , React.createElement('div', { style: { display:"flex", alignItems:"center", gap:8, cursor:"pointer" }, onClick: () => setActive("Home"),}
        , React.createElement('div', { style: { width:32, height:32, borderRadius:9, background:`linear-gradient(135deg,${C.teal},${C.lavender})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:16 },}, "🧩")
        , React.createElement('span', { style: { fontFamily:serif, fontSize:19, fontWeight:900, color:C.dark, letterSpacing:"-0.02em" },}, "Spectra", React.createElement('span', { style: { color:C.teal },}, "Guide"))
      )
      , !mobile && (
        React.createElement('div', { style: { display:"flex", gap:2 },}
          , NAV_MAIN.map(n => (
            React.createElement('button', { key: n, onClick: () => setActive(n), style: { background:active===n?`${C.teal}15`:"transparent", border:active===n?`1.5px solid ${C.teal}44`:"1.5px solid transparent", borderRadius:9, padding:"7px 11px", fontSize:13, fontWeight:active===n?700:500, color:active===n?C.teal:C.mid, cursor:"pointer", fontFamily:font, transition:"all 0.18s" },}, t[n.toLowerCase()]||n)
          ))
        )
      )
      , React.createElement('div', { style: { display:"flex", gap:8, alignItems:"center" },}
        , React.createElement('button', { onClick: () => setLang(lang==="en"?"es":"en"), style: { background:"none", border:`1.5px solid ${C.border}`, borderRadius:8, padding:"5px 10px", fontSize:12, fontWeight:700, color:C.mid, cursor:"pointer", fontFamily:font },}, lang==="en"?"🌐 ES":"🌐 EN")
        , user ? (
          React.createElement('div', { style: { display:"flex", gap:8, alignItems:"center" },}
            , React.createElement('button', { onClick: () => setActive("Dashboard"), style: { display:"flex", alignItems:"center", gap:7, background:`${C.teal}12`, border:`1.5px solid ${C.teal}33`, borderRadius:10, padding:"5px 12px", cursor:"pointer", fontFamily:font },}
              , React.createElement('div', { style: { width:24, height:24, borderRadius:"50%", background:`linear-gradient(135deg,${C.teal},${C.lavender})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, color:"white", fontWeight:700 },}, _optionalChain([user, 'access', _6 => _6.name, 'optionalAccess', _7 => _7[0], 'optionalAccess', _8 => _8.toUpperCase, 'call', _9 => _9()]))
              , !mobile && React.createElement('span', { style: { fontSize:13, fontWeight:600, color:C.dark },}, user.name)
            )
            , user.isAdmin && React.createElement(Btn, { size: "sm", variant: "dark", onClick: () => setActive("Admin"),}, "⚙️ Admin" )
            , React.createElement(Btn, { variant: "ghost", size: "sm", onClick: () => setUser(null),}, t.signOut)
          )
        ) : (
          React.createElement(React.Fragment, null
            , !mobile && React.createElement(Btn, { variant: "ghost", size: "sm", onClick: () => setActive("Dashboard"),}, t.signIn)
            , React.createElement(Btn, { size: "sm", onClick: () => setActive("Pricing"),}, t.getStarted)
          )
        )
        , mobile && React.createElement('button', { onClick: () => setMOpen(!mOpen), style: { background:"none", border:"none", fontSize:22, cursor:"pointer", color:C.dark },}, "☰")
      )
      , mobile && mOpen && (
        React.createElement('div', { style: { position:"absolute", top:64, left:0, right:0, background:"white", borderBottom:`1px solid ${C.border}`, padding:"12px 0", boxShadow:"0 8px 24px rgba(0,0,0,0.1)" },}
          , NAV_MAIN.map(n => (
            React.createElement('button', { key: n, onClick: () => { setActive(n); setMOpen(false); }, style: { display:"block", width:"100%", textAlign:"left", background:"none", border:"none", padding:"12px 24px", fontSize:15, fontWeight:active===n?700:400, color:active===n?C.teal:C.dark, cursor:"pointer", fontFamily:font },}, t[n.toLowerCase()]||n)
          ))
        )
      )
    )
...
function HomePage({ setActive, waitlist, setWaitlist, t }) {
  ...
  return (
    React.createElement('div', { style: { paddingTop:64 },}
      /* HERO */
      , React.createElement('section', { style: { minHeight:"92vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", textAlign:"center", padding:mobile?"80px 20px 60px":"80px 32px 60px", background:`radial-gradient(ellipse at 20% 50%,${C.teal}12 0%,transparent 60%),radial-gradient(ellipse at 80% 20%,${C.lavender}18 0%,transparent 55%),${C.cream}`, position:"relative", overflow:"hidden" },}
        , React.createElement('div', { style: { position:"absolute", top:130, left:"6%", width:90, height:90, borderRadius:"50%", background:`${C.gold}22`, filter:"blur(4px)", pointerEvents:"none" },} )
        , React.createElement('div', { style: { position:"absolute", top:220, right:"8%", width:65, height:65, borderRadius:"50%", background:`${C.rose}20`, filter:"blur(3px)", pointerEvents:"none" },} )
        , React.createElement('div', { style: { display:"flex", gap:8, flexWrap:"wrap", justifyContent:"center", marginBottom:24 },}
          , ["🧩 Autism Advocacy","📋 IEP Analysis","🌍 Global Resources","💬 AI-Powered"].map(b => (
            React.createElement('span', { key: b, style: { background:"white", borderRadius:999, padding:"5px 14px", fontSize:12, fontWeight:600, color:C.mid, boxShadow:"0 2px 10px rgba(0,0,0,0.07)", border:`1.5px solid ${C.border}` },}, b)
          ))
        )
        , React.createElement('h1', { style: { fontFamily:serif, fontSize:mobile?"36px":"clamp(36px,5.5vw,68px)", fontWeight:900, color:C.dark, lineHeight:1.13, margin:"0 0 20px", maxWidth:820, letterSpacing:"-0.03em" },}
          , t.tagline.split(" ").slice(0,-2).join(" "), React.createElement('br', null )
          , React.createElement(GradText, { a: C.teal, b: C.lavender,}, t.tagline.split(" ").slice(-2).join(" "))
        )
        , React.createElement('p', { style: { fontSize:mobile?16:19, color:C.mid, maxWidth:560, lineHeight:1.75, margin:"0 0 32px" },}, t.heroSub)
        , React.createElement('div', { style: { display:"flex", gap:12, flexWrap:"wrap", justifyContent:"center", marginBottom:44 },}
          , React.createElement(Btn, { size: "lg", onClick: () => setActive("Chat"),}, "💬 " , t.start)
          , React.createElement(Btn, { size: "lg", variant: "secondary", onClick: () => setActive("IEP"),}, "📋 " , t.analyzeIEP)
        )
        /* Email capture */
        , React.createElement('div', { style: { background:"white", borderRadius:18, padding:"22px 26px", boxShadow:"0 8px 40px rgba(0,0,0,0.09)", maxWidth:460, width:"100%", border:`1px solid ${C.border}` },}
          , submitted ? (
            React.createElement('div', { style: { textAlign:"center" },}
              , React.createElement('div', { style: { fontSize:30, marginBottom:8 },}, "🎉")
              , React.createElement('div', { style: { fontWeight:800, fontSize:16, color:C.dark },}, "You're on the list!"   )
              , React.createElement('div', { style: { color:C.mid, fontSize:13, marginTop:6 },}, "Share your referral link and earn a free month:"        )
              , React.createElement('div', { style: { background:C.cream, borderRadius:9, padding:"8px 14px", marginTop:10, fontSize:12, fontWeight:700, color:C.teal, letterSpacing:"0.04em" },}, "spectraguide.com/ref/", refCode)
            )
          ) : (
            React.createElement(React.Fragment, null
              , React.createElement('div', { style: { fontWeight:700, fontSize:14, color:C.dark, marginBottom:4 },}, t.waitlistTitle)
              , React.createElement('div', { style: { color:C.soft, fontSize:12, marginBottom:14 },}, t.waitlistSub)
              , React.createElement('div', { style: { display:"flex", gap:9 },}
                , React.createElement(Input, { value: email, onChange: e => setEmail(e.target.value), placeholder: t.yourEmail, type: "email", style: { flex:1 },} )
                , React.createElement(Btn, { onClick: joinWaitlist, disabled: !email.includes("@"),}, t.joinFree)
              )
            )
          )
        )
      ...
      /* STATS */
      , React.createElement('section', { style: { padding:"40px 32px", background:`linear-gradient(135deg,${C.dark},#3D3860)`, display:"flex", justifyContent:"center", gap:mobile?32:64, flexWrap:"wrap" },}
        , stats.map(s => (
          React.createElement('div', { key: s.v, style: { textAlign:"center" },}
            , React.createElement('div', { style: { fontFamily:serif, fontSize:36, fontWeight:900, color:"white", lineHeight:1 },}, s.v)
            , React.createElement('div', { style: { fontSize:12, color:"rgba(255,255,255,0.6)", marginTop:5 },}, s.l)
          )
        ))
      )

      /* FEATURES */
      , React.createElement('section', { style: { padding:"72px 24px", maxWidth:1100, margin:"0 auto" },}
        , React.createElement('div', { style: { textAlign:"center", marginBottom:48 },}
          , React.createElement(Pill, null, "FEATURES")
          , React.createElement('h2', { style: { fontFamily:serif, fontSize:mobile?28:36, fontWeight:800, color:C.dark, margin:"14px 0 10px", letterSpacing:"-0.02em" },}, "Everything in one place"   )
        )
        , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr":"repeat(auto-fit,minmax(290px,1fr))", gap:20 },}
          , features.map(f => (
            React.createElement(Card, { key: f.label, onClick: () => setActive(f.tab),}
              , React.createElement('div', { style: { width:48, height:48, borderRadius:13, background:`${f.color}18`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, marginBottom:14 },}, f.icon)
              , React.createElement('h3', { style: { fontSize:16, fontWeight:800, color:C.dark, margin:"0 0 7px" },}, f.label)
              , React.createElement('p', { style: { fontSize:13.5, color:C.mid, lineHeight:1.65, margin:"0 0 14px" },}, f.desc)
              , React.createElement('button', { onClick: e => { e.stopPropagation(); setActive(f.tab); }, style: { background:"none", border:`1.5px solid ${f.color}55`, borderRadius:8, padding:"6px 14px", fontSize:12, fontWeight:700, color:f.color, cursor:"pointer", fontFamily:font },}, "Explore →" )
            )
          ))
        )
      )
      ...
      /* TESTIMONIALS */
      , React.createElement('section', { style: { padding:"64px 24px", background:`${C.teal}07` },}
        , React.createElement('div', { style: { maxWidth:1050, margin:"0 auto", textAlign:"center" },}
          , React.createElement(Pill, { color: C.lavender,}, "COMMUNITY")
          , React.createElement('h2', { style: { fontFamily:serif, fontSize:mobile?26:32, fontWeight:800, color:C.dark, margin:"14px 0 36px", letterSpacing:"-0.02em" },}, "Voices from our community"   )
          , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr":"repeat(3,1fr)", gap:20 },}
            , testimonials.map(t => (
              React.createElement(Card, { key: t.n,}
                , React.createElement('div', { style: { fontSize:26, color:C.teal, marginBottom:12 },}, "\"")
                , React.createElement('p', { style: { fontSize:14, color:C.mid, lineHeight:1.72, fontStyle:"italic", margin:"0 0 18px" },}, t.q)
                , React.createElement('div', { style: { display:"flex", alignItems:"center", gap:10 },}
                  , React.createElement('div', { style: { width:40, height:40, borderRadius:"50%", background:`linear-gradient(135deg,${C.teal}22,${C.lavender}22)`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:18 },}, t.a)
                  , React.createElement('div', null, React.createElement('div', { style: { fontWeight:700, fontSize:13, color:C.dark },}, t.n), React.createElement('div', { style: { fontSize:11, color:C.soft },}, t.r))
                )
              )
            ))
          )
      ...
      /* CTA */
      , React.createElement('section', { style: { padding:"72px 24px", textAlign:"center", background:`linear-gradient(135deg,${C.dark},#3D3860)` },}
        , React.createElement('h2', { style: { fontFamily:serif, fontSize:mobile?32:42, fontWeight:900, color:"white", margin:"0 0 14px", letterSpacing:"-0.02em" },}, "Your journey starts "
             , React.createElement(GradText, { a: C.teal, b: C.gold,}, "here.")
        )
        , React.createElement('p', { style: { color:"rgba(255,255,255,0.6)", fontSize:16, maxWidth:400, margin:"0 auto 32px" },}, "Join thousands of families who've found their voice."       )
        , React.createElement('div', { style: { display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap" },}
          , React.createElement(Btn, { size: "lg", variant: "gold", onClick: () => setActive("Pricing"),}, "See Plans 💎"  )
          , React.createElement(Btn, { size: "lg", variant: "secondary", onClick: () => setActive("Booking"),}, "📅 Book a Demo"   )
        )
      )
    )
  );
...
function AdvocateChat({ user, chatHistory, setChatHistory, lang }) {
  ...
  useEffect(() => { _optionalChain([bottomRef, 'access', _10 => _10.current, 'optionalAccess', _11 => _11.scrollIntoView, 'call', _12 => _12({ behavior:"smooth" })]); }, [messages, loading]);
  useEffect(() => {
...
Siempre reconoce las emociones primero. Explica el vocabulario técnico. Usa estructura para mayor claridad. Nunca diagnostiques.`;
  ...
  async function send(text) {
    ...
    } catch (e4) { setMessages(m => [...m, { role:"assistant", content:"Sorry, something went wrong. Please try again 💙" }]); }
    setLoading(false);
  }

  return (
    React.createElement('div', { style: { paddingTop:64, height:"100vh", display:"flex", flexDirection:"column", background:C.cream },}
      , React.createElement('div', { style: { padding:"12px 20px", background:"white", borderBottom:`1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"space-between" },}
        , React.createElement('div', { style: { display:"flex", alignItems:"center", gap:10 },}
          , React.createElement('div', { style: { width:42, height:42, borderRadius:12, background:`linear-gradient(135deg,${C.teal},${C.lavender})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:19 },}, "🤝")
          , React.createElement('div', null
            , React.createElement('div', { style: { display:"flex", alignItems:"center", gap:7 },}
              , React.createElement('div', { style: { fontWeight:800, fontSize:15, color:C.dark },}, "SpectraGuide Advocate" )
              , React.createElement('span', { style: { background:`linear-gradient(135deg,${C.lavender},${C.teal})`, color:"white", fontSize:9, fontWeight:700, padding:"2px 8px", borderRadius:999 },}, "OPUS")
              , lang==="es" && React.createElement('span', { style: { background:`${C.gold}22`, color:C.peach, fontSize:9, fontWeight:700, padding:"2px 8px", borderRadius:999, border:`1px solid ${C.gold}44` },}, "ES")
            )
            , React.createElement('div', { style: { fontSize:11, color:C.teal, fontWeight:600 },}, "● Online" )
          )
        )
        , user && React.createElement(Btn, { variant: "ghost", size: "sm", onClick: () => { const r=[{role:"assistant",content:"Hi! I'm ready to help 💙"}]; setMessages(r); setChatHistory([]); },}, "Clear")
      )
      , React.createElement('div', { style: { padding:"8px 18px", background:"white", borderBottom:`1px solid ${C.border}88`, display:"flex", gap:6, flexWrap:"wrap" },}
        , (lang==="es"?QUICK_ES:QUICK_EN).map(q => (
          React.createElement('button', { key: q, onClick: () => send(q), style: { background:`${C.teal}10`, border:`1.5px solid ${C.teal}2A`, borderRadius:999, padding:"4px 11px", fontSize:11, fontWeight:600, color:C.teal, cursor:"pointer", fontFamily:font },}, q)
        ))
      )
      , React.createElement('div', { style: { flex:1, overflowY:"auto", padding:"18px 20px", display:"flex", flexDirection:"column", gap:12 },}
        , messages.map((m,i) => (
          React.createElement('div', { key: i, style: { display:"flex", justifyContent:m.role==="user"?"flex-end":"flex-start" },}
            , m.role==="assistant" && React.createElement('div', { style: { width:30, height:30, borderRadius:8, background:`linear-gradient(135deg,${C.teal},${C.lavender})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, marginRight:8, flexShrink:0, marginTop:4 },}, "🧩")
            , React.createElement('div', { style: { maxWidth:mobile?"85%":"68%", padding:"12px 16px", borderRadius:m.role==="user"?"16px 16px 4px 16px":"16px 16px 16px 4px", background:m.role==="user"?`linear-gradient(135deg,${C.teal},${C.sky})`:"white", color:m.role==="user"?"white":C.dark, fontSize:14, lineHeight:1.72, boxShadow:"0 2px 8px rgba(0,0,0,0.06)", whiteSpace:"pre-wrap" },}, m.content)
          )
        ))
        , loading && (
          React.createElement('div', { style: { display:"flex", gap:8 },}
            , React.createElement('div', { style: { width:30, height:30, borderRadius:8, background:`linear-gradient(135deg,${C.teal},${C.lavender})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:14 },}, "🧩")
            , React.createElement('div', { style: { background:"white", borderRadius:"16px 16px 16px 4px", padding:"12px 16px", boxShadow:"0 2px 8px rgba(0,0,0,0.06)", display:"flex", alignItems:"center", gap:8 },}
              , [0,1,2].map(d => React.createElement('div', { key: d, style: { width:6, height:6, borderRadius:"50%", background:C.teal, animation:`bounce 1.2s ${d*0.2}s infinite` },} ))
              , React.createElement('span', { style: { fontSize:11, color:C.soft, fontStyle:"italic" },}, thinking)
            )
          )
        )
        , React.createElement('div', { ref: bottomRef,} )
      )
      , React.createElement('div', { style: { padding:"12px 18px", background:"white", borderTop:`1px solid ${C.border}`, display:"flex", gap:9 },}
        , React.createElement('input', { value: input, onChange: e => setInput(e.target.value), onKeyDown: e => e.key==="Enter"&&!e.shiftKey&&send(), placeholder: lang==="es"?"Pregunta sobre IEPs, derechos, recursos, terapias...":"Ask about IEPs, autism rights, resources, therapies...", style: { flex:1, border:`1.5px solid ${C.border}`, borderRadius:11, padding:"11px 14px", fontSize:14, fontFamily:font, background:C.cream, color:C.dark, outline:"none" },} )
        , React.createElement(Btn, { onClick: () => send(), disabled: loading||!input.trim(),}, "Send →" )
      )
      , React.createElement('style', null, `@keyframes bounce{0%,80%,100%{transform:translateY(0)}40%{transform:translateY(-7px)}}`)
    )
  );
}
...
{"documentType":"IEP or BIP","studentName":"name or null","studentAge":"age/grade or null","disability":"disability category or null","overallScore":1-10,"scoreRationale":"1-2 sentences","summary":"3-4 sentence plain-language summary","strengths":["..."],"gaps":["specific gap with why it matters"],"redFlags":["serious concern with IDEA citation if applicable"],"goalAnalysis":["assessment of each goal"],"servicesReview":["assessment of each service"],"parentRights":["specific right with explanation"],"recommendations":["specific actionable recommendation"],"questionsToAsk":["pointed question for school team"],"nextSteps":["prioritized action step"],"legalConcerns":["potential IDEA/FAPE/LRE violation or null array"]}`, `Analyze this document:\n\n${docText}`, 2500);
    ...
    } catch (e5) { setAnalysis({ error:true }); }
    setLoading(false);
  }

  return (
    React.createElement('div', { style: { paddingTop:80, minHeight:"100vh", background:C.cream, padding:"80px 20px 60px" },}
      , React.createElement('div', { style: { maxWidth:900, margin:"0 auto" },}
        , React.createElement('div', { style: { textAlign:"center", marginBottom:32 },}
          , React.createElement(Pill, { color: C.lavender,}, "IEP & BIP ANALYZER"   )
          , React.createElement('h1', { style: { fontFamily:serif, fontSize:mobile?28:34, fontWeight:900, color:C.dark, margin:"14px 0 10px", letterSpacing:"-0.02em" },}, "Understand your child's plan"   )
          , React.createElement('p', { style: { color:C.mid, fontSize:15, maxWidth:500, margin:"0 auto" },}, "Get a plain-language breakdown with strengths, gaps, red flags, and your rights."           )
        )

        , user && iepHistory.length>0 && (
          React.createElement('div', { style: { display:"flex", gap:8, marginBottom:18 },}
            , React.createElement(Btn, { variant: tab==="analyze"?"primary":"ghost", size: "sm", onClick: () => setTab("analyze"),}, "📋 New Analysis"  )
            , React.createElement(Btn, { variant: tab==="history"?"primary":"ghost", size: "sm", onClick: () => setTab("history"),}, "🕐 History ("  , iepHistory.length, ")")
          )
        )
        ...
        , tab==="history" && (
          React.createElement('div', { style: { marginBottom:24 },}
            , iepHistory.map((h,i) => (
              React.createElement(Card, { key: i, style: { cursor:"pointer", marginBottom:12 }, onClick: () => { setAnalysis(h.result); setTab("analyze"); },}
                , React.createElement('div', { style: { display:"flex", justifyContent:"space-between", alignItems:"center" },}
                  , React.createElement('div', null
                    , React.createElement('div', { style: { fontWeight:700, fontSize:14, color:C.dark },}, _optionalChain([h, 'access', _13 => _13.result, 'optionalAccess', _14 => _14.documentType]), " — Score "   , _optionalChain([h, 'access', _15 => _15.result, 'optionalAccess', _16 => _16.overallScore]), "/10")
                    , React.createElement('div', { style: { fontSize:12, color:C.soft, marginTop:3 },}, h.text)
                  )
                  , React.createElement('div', { style: { fontSize:12, color:C.soft },}, h.date)
                )
              )
            ))
          )
        ...
        , React.createElement(Card, { style: { marginBottom:20 },}
          , React.createElement('div', { style: { display:"flex", gap:8, marginBottom:12, flexWrap:"wrap" },}
            , React.createElement(Btn, { variant: "ghost", size: "sm", onClick: () => setDocText(SAMPLE),}, "✨ Try Sample IEP"   )
            , React.createElement(Btn, { variant: "ghost", size: "sm", onClick: () => setDocText(""),}, "🗑️ Clear" )
          )
          , React.createElement('textarea', { value: docText, onChange: e => setDocText(e.target.value), placeholder: "Paste your IEP or BIP text here... Include goals, services, present levels, accommodations, and parent concerns."               , style: { width:"100%", minHeight:200, border:`1.5px solid ${C.border}`, borderRadius:12, padding:"13px 15px", fontSize:14, fontFamily:font, background:C.cream, color:C.dark, outline:"none", resize:"vertical", lineHeight:1.65, boxSizing:"border-box" },} )
          , React.createElement('div', { style: { display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:10, flexWrap:"wrap", gap:10 },}
            , React.createElement('span', { style: { fontSize:12, color:C.soft },}, docText.length, " characters" )
            , React.createElement('div', { style: { display:"flex", gap:8 },}
              , analysis && !analysis.error && React.createElement(Btn, { variant: "ghost", size: "sm", onClick: () => exportIEPReport(analysis),}, "📄 Export PDF"  )
              , React.createElement(Btn, { onClick: analyze, disabled: loading||!docText.trim(), style: { background:`linear-gradient(135deg,${C.lavender},${C.sky})`, boxShadow:`0 6px 20px ${C.lavender}44` },}, loading?"Analyzing…":"🔍 Analyze")
            )
          )
        )

        , loading && React.createElement(Card, { style: { textAlign:"center", padding:48 },}, React.createElement('div', { style: { fontSize:36, marginBottom:12 },}, "🧩"), React.createElement('div', { style: { fontWeight:700, fontSize:17, color:C.dark },}, "Reading your document…"  ), React.createElement('div', { style: { color:C.mid, fontSize:13, marginTop:6 },}, "Checking goals, services, rights, and potential gaps with Claude Opus."         ))

        , analysis && !analysis.error && (
          React.createElement('div', { style: { display:"flex", flexDirection:"column", gap:16 },}
            , React.createElement(Card, { style: { background:`linear-gradient(135deg,${C.dark},#3D3860)` },}
              , React.createElement('div', { style: { display:"flex", justifyContent:"space-between", flexWrap:"wrap", gap:16, alignItems:"center" },}
                , React.createElement('div', { style: { flex:1 },}
                  , React.createElement('div', { style: { color:"rgba(255,255,255,0.5)", fontSize:10, fontWeight:700, letterSpacing:"0.08em", marginBottom:4 },}, "DOCUMENT")
                  , React.createElement('div', { style: { color:"white", fontSize:20, fontWeight:800 },}, analysis.documentType, analysis.studentName?` — ${analysis.studentName}`:"")
                  , React.createElement('div', { style: { display:"flex", gap:6, marginTop:6, flexWrap:"wrap" },}
                    , analysis.studentAge && React.createElement('span', { style: { background:`${C.teal}33`, color:C.teal, fontSize:10, fontWeight:700, padding:"2px 9px", borderRadius:999 },}, analysis.studentAge)
                    , analysis.disability && React.createElement('span', { style: { background:`${C.lavender}33`, color:C.lavender, fontSize:10, fontWeight:700, padding:"2px 9px", borderRadius:999 },}, analysis.disability)
                  )
                  , React.createElement('div', { style: { color:"rgba(255,255,255,0.7)", fontSize:13, lineHeight:1.65, marginTop:10, maxWidth:520 },}, analysis.summary)
                  , analysis.scoreRationale && React.createElement('div', { style: { color:"rgba(255,255,255,0.45)", fontSize:11, fontStyle:"italic", marginTop:6 },}, analysis.scoreRationale)
                )
                , React.createElement('div', { style: { textAlign:"center" },}
                  , React.createElement('div', { style: { width:80, height:80, borderRadius:"50%", background:`conic-gradient(${C.teal} ${analysis.overallScore*36}deg,rgba(255,255,255,0.12) 0)`, display:"flex", alignItems:"center", justifyContent:"center" },}
                    , React.createElement('div', { style: { width:62, height:62, borderRadius:"50%", background:"#2D2A3E", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center" },}
                      , React.createElement('div', { style: { color:"white", fontSize:22, fontWeight:900, lineHeight:1 },}, analysis.overallScore)
                      , React.createElement('div', { style: { color:"rgba(255,255,255,0.4)", fontSize:9 },}, "/10")
                    )
                  )
                  , React.createElement('div', { style: { color:"rgba(255,255,255,0.45)", fontSize:10, marginTop:5 },}, "Score")
                )
              )
              , React.createElement('div', { style: { marginTop:14, display:"flex", justifyContent:"flex-end" },}
                , React.createElement(Btn, { size: "sm", variant: "ghost", onClick: () => exportIEPReport(analysis), style: { borderColor:"rgba(255,255,255,0.3)", color:"white" },}, "📄 Export Full Report"   )
              )
            )

            , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr":"1fr 1fr", gap:16 },}
              , [{ label:"✅ Strengths", items:analysis.strengths, color:C.teal },{ label:"⚠️ Gaps", items:analysis.gaps, color:C.peach },{ label:"🛡️ Your Rights", items:analysis.parentRights, color:C.lavender },{ label:"💡 Recommendations", items:analysis.recommendations, color:C.sky }].map(s => (
                React.createElement(Card, { key: s.label,}
                  , React.createElement('div', { style: { fontSize:13, fontWeight:800, color:C.dark, marginBottom:10 },}, s.label)
                  , _optionalChain([s, 'access', _17 => _17.items, 'optionalAccess', _18 => _18.map, 'call', _19 => _19((item,i) => React.createElement('div', { key: i, style: { display:"flex", gap:8, marginBottom:7 },}, React.createElement('div', { style: { width:5, height:5, borderRadius:"50%", background:s.color, marginTop:8, flexShrink:0 },} ), React.createElement('div', { style: { fontSize:13, color:C.mid, lineHeight:1.55 },}, item)))])
                )
              ))
            )

            , _optionalChain([analysis, 'access', _20 => _20.goalAnalysis, 'optionalAccess', _21 => _21.length])>0 && (
              React.createElement(Card, { style: { background:`${C.teal}08` },}
                , React.createElement('div', { style: { fontSize:13, fontWeight:800, color:C.dark, marginBottom:10 },}, "🎯 Goal-by-Goal Analysis"  )
                , analysis.goalAnalysis.map((g,i) => (
                  React.createElement('div', { key: i, style: { display:"flex", gap:9, marginBottom:9, paddingBottom:9, borderBottom:i<analysis.goalAnalysis.length-1?`1px solid ${C.border}`:"none" },}
                    , React.createElement('div', { style: { width:20, height:20, borderRadius:"50%", background:C.teal, color:"white", fontSize:10, fontWeight:700, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 },}, i+1)
                    , React.createElement('div', { style: { fontSize:13, color:C.mid, lineHeight:1.55, paddingTop:1 },}, g)
                  )
                ))
              )
            )

            , _optionalChain([analysis, 'access', _22 => _22.servicesReview, 'optionalAccess', _23 => _23.length])>0 && (
              React.createElement(Card, { style: { background:`${C.lavender}08` },}
                , React.createElement('div', { style: { fontSize:13, fontWeight:800, color:C.dark, marginBottom:10 },}, "🏥 Services Review"  )
                , analysis.servicesReview.map((s,i) => React.createElement('div', { key: i, style: { display:"flex", gap:8, marginBottom:7 },}, React.createElement('div', { style: { width:5, height:5, borderRadius:"50%", background:C.lavender, marginTop:8, flexShrink:0 },} ), React.createElement('div', { style: { fontSize:13, color:C.mid, lineHeight:1.55 },}, s)))
              )
            )

            , _optionalChain([analysis, 'access', _24 => _24.redFlags, 'optionalAccess', _25 => _25.length])>0 && (
              React.createElement(Card, { style: { background:`${C.rose}0D`, border:`1.5px solid ${C.rose}33` },}
                , React.createElement('div', { style: { fontSize:13, fontWeight:800, color:C.rose, marginBottom:9 },}, "🚨 Red Flags"  )
                , analysis.redFlags.map((f,i) => React.createElement('div', { key: i, style: { display:"flex", gap:7, marginBottom:6 },}, React.createElement('span', { style: { color:C.rose, fontWeight:700 },}, "!"), React.createElement('span', { style: { fontSize:13, color:C.dark },}, f)))
              )
            )

            , _optionalChain([analysis, 'access', _26 => _26.legalConcerns, 'optionalAccess', _27 => _27.length])>0 && (
              React.createElement(Card, { style: { background:`${C.gold}0A`, border:`1.5px solid ${C.gold}44` },}
                , React.createElement('div', { style: { fontSize:13, fontWeight:800, color:C.peach, marginBottom:9 },}, "⚖️ Legal Concerns"  )
                , analysis.legalConcerns.map((l,i) => React.createElement('div', { key: i, style: { display:"flex", gap:7, marginBottom:6 },}, React.createElement('span', { style: { color:C.gold, fontWeight:700 },}, "§"), React.createElement('span', { style: { fontSize:13, color:C.dark },}, l)))
              )
            )

            , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr":"1fr 1fr", gap:16 },}
              , React.createElement(Card, { style: { background:`${C.teal}08` },}
                , React.createElement('div', { style: { fontSize:13, fontWeight:800, color:C.dark, marginBottom:9 },}, "🗺️ Next Steps"  )
                , _optionalChain([analysis, 'access', _28 => _28.nextSteps, 'optionalAccess', _29 => _29.map, 'call', _30 => _30((s,i) => React.createElement('div', { key: i, style: { display:"flex", gap:9, marginBottom:9 },}, React.createElement('div', { style: { width:20, height:20, borderRadius:"50%", background:C.teal, color:"white", fontSize:10, fontWeight:700, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 },}, i+1), React.createElement('div', { style: { fontSize:13, color:C.mid, lineHeight:1.55, paddingTop:1 },}, s)))])
              )
              , React.createElement(Card, { style: { background:`${C.lavender}08` },}
                , React.createElement('div', { style: { fontSize:13, fontWeight:800, color:C.dark, marginBottom:9 },}, "❓ Questions to Ask"   )
                , _optionalChain([analysis, 'access', _31 => _31.questionsToAsk, 'optionalAccess', _32 => _32.map, 'call', _33 => _33((q,i) => React.createElement('div', { key: i, style: { display:"flex", gap:8, marginBottom:7 },}, React.createElement('span', { style: { color:C.lavender, fontWeight:700 },}, "?"), React.createElement('div', { style: { fontSize:13, color:C.mid, lineHeight:1.55 },}, q)))])
              )
            )
          )
        )
        , _optionalChain([analysis, 'optionalAccess', _34 => _34.error]) && React.createElement(Card, { style: { textAlign:"center", padding:36 },}, React.createElement('div', { style: { fontSize:30, marginBottom:10 },}, "😔"), React.createElement('div', { style: { fontWeight:700, color:C.dark },}, "Analysis failed" ), React.createElement('div', { style: { color:C.mid, fontSize:14, marginTop:6 },}, "Please try again — paste more of the document for best results."           ))
      )
    )
  );
}
...
function ResourceFinder({ user, savedResources, setSavedResources, lang }) {
  ...
  const renderResource = (r, i) => {
    ...
    return (
      React.createElement(Card, { key: r.name + i,}
        , React.createElement('div', { style: { display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:7 },}
          , React.createElement('div', { style: { display:"flex", gap:6, flexWrap:"wrap" },}
            , React.createElement(Tag, { c: SCOPE_C[sc] || C.teal,}, sc)
            , r.free === true && React.createElement(Tag, { c: C.teal,}, "Free")
          )
          , user && (
            React.createElement('button', { onClick: () => toggleSave(r), style: { background:"none", border:"none", cursor:"pointer", fontSize:17 },}
              , savedResources.find(s => s.name === r.name) ? "❤️" : "🤍"
            )
          )
        )
        , React.createElement('h3', { style: { fontSize:15, fontWeight:800, color:C.dark, margin:"0 0 4px" },}, r.name)
        , r.type && React.createElement('div', { style: { fontSize:10, color:C.soft, fontWeight:600, marginBottom:7 },}, r.type)
        , r.description && React.createElement('p', { style: { fontSize:13, color:C.mid, lineHeight:1.65, margin:"0 0 9px" },}, r.description)
        , r.tips && (
          React.createElement('div', { style: { background:`${C.gold}18`, borderRadius:7, padding:"6px 10px", marginBottom:10 },}
            , React.createElement('span', { style: { fontSize:10, fontWeight:700, color:C.peach },}, "💡 " )
            , React.createElement('span', { style: { fontSize:10, color:C.mid },}, r.tips)
          )
        )
        , r.waitlist && React.createElement('div', { style: { fontSize:10, color:C.soft, marginBottom:8 },}, "⏳ " , r.waitlist)
        , React.createElement('div', { style: { display:"flex", gap:10, flexWrap:"wrap" },}
          , r.link && String(r.link).startsWith("http") && (
            React.createElement('a', { href: r.link, target: "_blank", rel: "noopener noreferrer" ,
              style: { fontSize:12, fontWeight:700, color:C.teal, textDecoration:"none", borderBottom:`1px solid ${C.teal}44` },}, "Visit Site →"

            )
          )
          , r.phone && React.createElement('span', { style: { fontSize:11, color:C.mid },}, "📞 " , r.phone)
        )
      )
    );
  };
  ...
  return (
    React.createElement('div', { style: { paddingTop:80, minHeight:"100vh", background:C.cream, padding:"80px 20px 60px" },}
      , React.createElement('div', { style: { maxWidth:980, margin:"0 auto" },}
        ...
        , React.createElement('div', { style: { textAlign:"center", marginBottom:32 },}
          , React.createElement(Pill, { color: C.peach,}, "RESOURCE FINDER" )
          , React.createElement('h1', { style: { fontFamily:serif, fontSize:mobile?26:32, fontWeight:900, color:C.dark, margin:"12px 0 8px", letterSpacing:"-0.02em" },}, "Find autism support anywhere"

          )
          , React.createElement('p', { style: { color:C.mid, fontSize:15, maxWidth:540, margin:"0 auto" },}, "Trusted directories to find local providers in all 50 states, plus our national resource database."
        ...
        , React.createElement('div', { style: { marginBottom:40 },}
          , React.createElement('div', { style: { display:"flex", alignItems:"center", gap:10, marginBottom:6 },}
            , React.createElement('div', { style: { fontWeight:800, fontSize:16, color:C.dark },}, "📍 Find Local Providers — Any City, Any State"        )
            , React.createElement('span', { style: { background:`${C.teal}18`, color:C.teal, fontSize:10, fontWeight:700, padding:"3px 10px", borderRadius:999, border:`1px solid ${C.teal}33` },}, "ALL 50 STATES"  )
          )
          , React.createElement('p', { style: { fontSize:13, color:C.mid, marginBottom:18, lineHeight:1.65 },}, "Click any directory below to search for verified providers in your city. Each one lets you search by zip code, filter by insurance, and see real reviews."

          )
          , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr 1fr":"repeat(auto-fill,minmax(260px,1fr))", gap:12 },}
            , DIRECTORIES.map((d, i) => (
              React.createElement('a', { key: i, href: d.url, target: "_blank", rel: "noopener noreferrer" , style: { textDecoration:"none" },}
                , React.createElement('div', { style: { background:"white", border:`1.5px solid ${d.color}33`, borderRadius:14, padding:"14px 16px",
                  boxShadow:"0 2px 12px rgba(0,0,0,0.05)", cursor:"pointer", height:"100%", display:"flex", flexDirection:"column", gap:8,
                  transition:"all 0.15s" },}
                  , React.createElement('div', { style: { display:"flex", alignItems:"center", gap:9 },}
                    , React.createElement('div', { style: { width:36, height:36, borderRadius:10, background:`${d.color}18`,
                      display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, flexShrink:0 },}
                      , d.icon
                    )
                    , React.createElement('div', { style: { fontWeight:800, fontSize:13, color:C.dark, lineHeight:1.3 },}, d.name)
                  )
                  , React.createElement('div', { style: { fontSize:11.5, color:C.mid, lineHeight:1.6, flex:1 },}, d.desc)
                  , React.createElement('div', { style: { fontSize:11, fontWeight:700, color:d.color },}, "Open & search by zip code →"      )
                )
              )
            ))
          )
        ...
        , React.createElement('div', null
          , React.createElement('div', { style: { fontWeight:800, fontSize:16, color:C.dark, marginBottom:6 },}, "🌍 National Resource Database"   )
          , React.createElement('p', { style: { fontSize:13, color:C.mid, marginBottom:16 },}, "Filter by service type or search by name. These organizations serve families in every state."
          ...
          , React.createElement('div', { style: { display:"flex", gap:6, flexWrap:"wrap", marginBottom:14 },}
            , CATS.map(c => (
              React.createElement('button', { key: c.q, onClick: () => setFilter(c.q),
          ...
          , user && savedResources.length > 0 && (
            React.createElement('div', { style: { display:"flex", gap:8, marginBottom:16 },}
              , React.createElement(Btn, { variant: tab==="search"?"primary":"ghost", size: "sm", onClick: () => setTab("search"),}, "Resources")
              , React.createElement(Btn, { variant: tab==="saved"?"primary":"ghost", size: "sm", onClick: () => setTab("saved"),}, "❤️ Saved ("  , savedResources.length, ")")
            )
          )

          , tab === "saved" && (
            React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr":"repeat(auto-fit,minmax(340px,1fr))", gap:16 },}
          ...
          , tab === "search" && (
            queryFiltered.length === 0 ? (
              React.createElement(Card, { style: { textAlign:"center", padding:32 },}
                , React.createElement('div', { style: { fontSize:13, color:C.mid },}, "No resources match that search. Try a different term."        )
              )
            ) : (
              React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr":"repeat(auto-fit,minmax(340px,1fr))", gap:16 },}
...
function BlogHub() {
  ...
  async function loadPost(post) {
    ...
    } catch (e7) { setSelected({...post, content:"Unable to load article. Please try again."}); }
    setPostLoading(false);
  }

  useEffect(() => { loadPosts(); }, []);
  const filtered = _optionalChain([posts, 'optionalAccess', _35 => _35.filter, 'call', _36 => _36(p => cat==="All"||p.category===cat)]);

  if (selected) return (
    React.createElement('div', { style: { paddingTop:80, minHeight:"100vh", background:C.cream, padding:"80px 20px 60px" },}
      , React.createElement('div', { style: { maxWidth:720, margin:"0 auto" },}
        , React.createElement(Btn, { variant: "ghost", size: "sm", onClick: ()=>setSelected(null), style: { marginBottom:24 },}, "← Back" )
        , React.createElement(Tag, { c: C.sky,}, selected.category)
        , React.createElement('h1', { style: { fontFamily:serif, fontSize:mobile?26:34, fontWeight:900, color:C.dark, margin:"12px 0 14px", lineHeight:1.2 },}, selected.emoji, " " , selected.title)
        , React.createElement('div', { style: { display:"flex", gap:12, fontSize:12, color:C.soft, marginBottom:28 },}
          , React.createElement('span', null, "✍️ " , selected.author), React.createElement('span', null, "⏱ " , selected.readTime)
        )
        , postLoading ? React.createElement(Card, { style: { padding:48, textAlign:"center" },}, React.createElement('div', { style: { fontSize:30, marginBottom:10 },}, "📖"), React.createElement('div', { style: { fontWeight:700, color:C.dark },}, "Writing article with Claude Opus…"    ))
          : React.createElement(Card, null, React.createElement('div', { style: { fontSize:15, lineHeight:1.88, color:C.mid, whiteSpace:"pre-wrap" },}, selected.content))
      )
    )
  );
  ...
  return (
    React.createElement('div', { style: { paddingTop:80, minHeight:"100vh", background:C.cream, padding:"80px 20px 60px" },}
      , React.createElement('div', { style: { maxWidth:1050, margin:"0 auto" },}
        , React.createElement('div', { style: { textAlign:"center", marginBottom:36 },}
          , React.createElement(Pill, { color: C.sky,}, "LEARNING HUB" )
          , React.createElement('h1', { style: { fontFamily:serif, fontSize:mobile?28:34, fontWeight:900, color:C.dark, margin:"12px 0 8px", letterSpacing:"-0.02em" },}, "Knowledge is your greatest tool"    )
        )
        , React.createElement('div', { style: { display:"flex", gap:6, flexWrap:"wrap", justifyContent:"center", marginBottom:28 },}
          , CATS.map(c => React.createElement('button', { key: c, onClick: ()=>setCat(c), style: { background:cat===c?`${C.sky}18`:"transparent", border:`1.5px solid ${cat===c?C.sky:C.border}`, borderRadius:999, padding:"6px 14px", fontSize:12, fontWeight:cat===c?700:500, color:cat===c?C.sky:C.mid, cursor:"pointer", fontFamily:font },}, c))
        )
        , loading && React.createElement(Card, { style: { textAlign:"center", padding:44 },}, React.createElement('div', { style: { fontSize:30 },}, "📖"), React.createElement('div', { style: { fontWeight:700, color:C.dark, marginTop:10 },}, "Loading articles…" ))
        , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr":"repeat(auto-fit,minmax(290px,1fr))", gap:18 },}
          , _optionalChain([filtered, 'optionalAccess', _37 => _37.map, 'call', _38 => _38(post => (
            React.createElement(Card, { key: post.id, style: { cursor:"pointer" }, onClick: ()=>loadPost(post),}
              , React.createElement('div', { style: { fontSize:32, marginBottom:12 },}, post.emoji)
              , React.createElement(Tag, { c: C.sky,}, post.category)
              , React.createElement('h3', { style: { fontSize:15, fontWeight:800, color:C.dark, margin:"9px 0 7px", lineHeight:1.35 },}, post.title)
              , React.createElement('p', { style: { fontSize:13, color:C.mid, lineHeight:1.65, margin:"0 0 14px" },}, post.excerpt)
              , React.createElement('div', { style: { display:"flex", justifyContent:"space-between", alignItems:"center" },}
                , React.createElement('div', { style: { fontSize:11, color:C.soft },}, post.author, " · "  , post.readTime)
                , React.createElement('span', { style: { fontSize:12, fontWeight:700, color:C.sky },}, "Read →" )
              )
            )
          ))])
        )
...
function PricingPage({ setActive }) {
  ...
  return (
    React.createElement('div', { style: { paddingTop:80, minHeight:"100vh", background:C.cream, padding:"80px 20px 80px" },}
      , React.createElement('div', { style: { maxWidth:1100, margin:"0 auto" },}
        , React.createElement('div', { style: { textAlign:"center", marginBottom:44 },}
          , React.createElement(Pill, { color: C.gold,}, "PRICING")
          , React.createElement('h1', { style: { fontFamily:serif, fontSize:mobile?28:38, fontWeight:900, color:C.dark, margin:"14px 0 10px", letterSpacing:"-0.02em" },}, "Simple, " , React.createElement(GradText, { a: C.teal, b: C.gold,}, "transparent"), " pricing" )
          , React.createElement('p', { style: { color:C.mid, fontSize:16, maxWidth:440, margin:"0 auto 24px" },}, "Start free. Upgrade when you're ready. Cancel anytime."       )
          , React.createElement('div', { style: { display:"inline-flex", background:"white", border:`1.5px solid ${C.border}`, borderRadius:12, padding:4, gap:4 },}
            , ["monthly","annual"].map(b => React.createElement('button', { key: b, onClick: ()=>setBilling(b), style: { background:billing===b?`linear-gradient(135deg,${C.teal},${C.sky})`:"transparent", border:"none", borderRadius:9, padding:"7px 18px", fontSize:13, fontWeight:700, color:billing===b?"white":C.mid, cursor:"pointer", fontFamily:font },}, b==="monthly"?"Monthly":"Annual (Save 20%)"))
          )
        )
        , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr":"repeat(auto-fit,minmax(220px,1fr))", gap:18, alignItems:"start" },}
          , plans.map(plan => (
            React.createElement('div', { key: plan.name, style: { position:"relative" },}
              , plan.popular && React.createElement('div', { style: { position:"absolute", top:-11, left:"50%", transform:"translateX(-50%)", background:`linear-gradient(135deg,${C.lavender},${C.sky})`, color:"white", fontSize:10, fontWeight:700, padding:"3px 14px", borderRadius:999, whiteSpace:"nowrap", zIndex:1 },}, "MOST POPULAR" )
              , React.createElement(Card, { style: { border:plan.popular?`2px solid ${C.lavender}`:`1px solid ${C.border}`, padding:24 },}
                , React.createElement('div', { style: { fontSize:26, marginBottom:8 },}, plan.emoji)
                , React.createElement('h3', { style: { fontSize:17, fontWeight:800, color:C.dark, margin:"0 0 4px" },}, plan.name)
                , React.createElement('p', { style: { fontSize:12, color:C.soft, margin:"0 0 14px" },}, plan.desc)
                , React.createElement('div', { style: { margin:"0 0 18px" },}
                  , React.createElement('span', { style: { fontFamily:serif, fontSize:34, fontWeight:900, color:plan.color },}, "$", plan.price[billing])
                  , React.createElement('span', { style: { fontSize:12, color:C.soft },}, plan.price[billing]>0?"/mo":" forever")
                )
                , React.createElement('div', { style: { marginBottom:20 },}
                  , plan.features.map((f,i) => React.createElement('div', { key: i, style: { display:"flex", gap:7, marginBottom:7 },}, React.createElement('span', { style: { color:plan.color, fontSize:13 },}, "✓"), React.createElement('span', { style: { fontSize:12.5, color:C.mid },}, f)))
                )
                , React.createElement('button', { onClick: ()=>setActive("Dashboard"), style: { width:"100%", background:plan.popular?`linear-gradient(135deg,${C.lavender},${C.sky})`:"white", border:plan.popular?"none":`2px solid ${C.border}`, borderRadius:11, padding:"11px 0", fontSize:13, fontWeight:700, color:plan.popular?"white":C.dark, cursor:"pointer", fontFamily:font, boxShadow:plan.popular?`0 6px 20px ${C.lavender}44`:"none" },}, plan.cta)
              )
            )
          ))
        )
        , React.createElement('div', { style: { marginTop:56, textAlign:"center" },}
          , React.createElement('h2', { style: { fontFamily:serif, fontSize:26, fontWeight:800, color:C.dark, margin:"0 0 28px" },}, "Frequently Asked Questions"  )
          , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr":"repeat(auto-fit,minmax(380px,1fr))", gap:14, textAlign:"left", maxWidth:900, margin:"0 auto" },}
            , [
              ...
              React.createElement(Card, { key: i,}
                , React.createElement('div', { style: { fontWeight:700, fontSize:13, color:C.dark, marginBottom:7 },}, "❓ " , faq.q)
                , React.createElement('div', { style: { fontSize:13, color:C.mid, lineHeight:1.65 },}, faq.a)
              )
            ))
          )
        )
...
function BookingPage({ bookings, setBookings }) {
  ...
  if (submitted) return (
    React.createElement('div', { style: { paddingTop:80, minHeight:"100vh", background:C.cream, display:"flex", alignItems:"center", justifyContent:"center", padding:"80px 20px" },}
      , React.createElement(Card, { style: { maxWidth:480, width:"100%", textAlign:"center", padding:48 },}
        , React.createElement('div', { style: { fontSize:48, marginBottom:16 },}, "🎉")
        , React.createElement('h2', { style: { fontFamily:serif, fontSize:26, fontWeight:900, color:C.dark, margin:"0 0 12px" },}, "Demo Booked!" )
        , React.createElement('p', { style: { color:C.mid, fontSize:15, lineHeight:1.7, marginBottom:24 },}, "We'll see you on "    , React.createElement('strong', null, form.date), " at "  , React.createElement('strong', null, form.time), ". A confirmation has been sent to "       , React.createElement('strong', null, form.email), ".")
        , React.createElement('p', { style: { fontSize:13, color:C.soft },}, "A SpectraGuide team member will send a meeting link within 24 hours."           )
      )
    )
  );
  ...
  return (
    React.createElement('div', { style: { paddingTop:80, minHeight:"100vh", background:C.cream, padding:"80px 20px 60px" },}
      , React.createElement('div', { style: { maxWidth:680, margin:"0 auto" },}
        , React.createElement('div', { style: { textAlign:"center", marginBottom:32 },}
          , React.createElement(Pill, { color: C.teal,}, "BOOK A DEMO"  )
          , React.createElement('h1', { style: { fontFamily:serif, fontSize:mobile?26:32, fontWeight:900, color:C.dark, margin:"14px 0 10px", letterSpacing:"-0.02em" },}, "See SpectraGuide in action"   )
          , React.createElement('p', { style: { color:C.mid, fontSize:15, maxWidth:440, margin:"0 auto" },}, "Schedule a personalized 30-minute demo for your school, district, or organization."          )
        )
        , React.createElement(Card, null
          , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr":"1fr 1fr", gap:14, marginBottom:14 },}
            , [{ label:"Full Name *", key:"name", placeholder:"Your name" },{ label:"Organization", key:"org", placeholder:"School or org name" },{ label:"Email *", key:"email", placeholder:"your@email.com", type:"email" },{ label:"Your Role", key:"role", select:true }].map(f => (
              React.createElement('div', { key: f.key,}
                , React.createElement('div', { style: { fontSize:12, fontWeight:700, color:C.mid, marginBottom:5 },}, f.label)
                , f.select ? (
                  React.createElement('select', { value: form[f.key], onChange: e=>setForm({...form,[f.key]:e.target.value}), style: { width:"100%", border:`1.5px solid ${C.border}`, borderRadius:10, padding:"10px 14px", fontSize:14, fontFamily:font, background:C.cream, color:C.dark, outline:"none" },}
                    , React.createElement('option', { value: "",}, "Select role…" )
                    , ROLES.map(r => React.createElement('option', { key: r,}, r))
                  )
                ) : (
                  React.createElement(Input, { value: form[f.key], onChange: e=>setForm({...form,[f.key]:e.target.value}), placeholder: f.placeholder, type: f.type||"text", style: { width:"100%", boxSizing:"border-box" },} )
                )
              )
            ))
          )
          , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr":"1fr 1fr", gap:14, marginBottom:14 },}
            , React.createElement('div', null
              , React.createElement('div', { style: { fontSize:12, fontWeight:700, color:C.mid, marginBottom:5 },}, "Preferred Date *"  )
              , React.createElement(Input, { value: form.date, onChange: e=>setForm({...form,date:e.target.value}), placeholder: "", type: "date", style: { width:"100%", boxSizing:"border-box" },} )
            )
            , React.createElement('div', null
              , React.createElement('div', { style: { fontSize:12, fontWeight:700, color:C.mid, marginBottom:5 },}, "Preferred Time" )
              , React.createElement('select', { value: form.time, onChange: e=>setForm({...form,time:e.target.value}), style: { width:"100%", border:`1.5px solid ${C.border}`, borderRadius:10, padding:"10px 14px", fontSize:14, fontFamily:font, background:C.cream, color:C.dark, outline:"none" },}
                , React.createElement('option', { value: "",}, "Select time (ET)…"  )
                , TIMES.map(t => React.createElement('option', { key: t,}, t))
              )
            )
          )
          , React.createElement('div', { style: { marginBottom:20 },}
            , React.createElement('div', { style: { fontSize:12, fontWeight:700, color:C.mid, marginBottom:5 },}, "What would you like to discuss? (optional)"      )
            , React.createElement('textarea', { value: form.message, onChange: e=>setForm({...form,message:e.target.value}), placeholder: "Tell us about your organization and what you're hoping to learn…"          , style: { width:"100%", minHeight:100, border:`1.5px solid ${C.border}`, borderRadius:10, padding:"11px 14px", fontSize:14, fontFamily:font, background:C.cream, color:C.dark, outline:"none", resize:"vertical", lineHeight:1.6, boxSizing:"border-box" },} )
          )
          , React.createElement(Btn, { onClick: submit, disabled: !form.name||!form.email||!form.date, style: { width:"100%" },}, "📅 Book My Demo"   )
        )
        , React.createElement('div', { style: { marginTop:28, display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:14 },}
          , [{ icon:"⏱", title:"30 Minutes", desc:"Focused, efficient demo" },{ icon:"🎯", title:"Personalized", desc:"Tailored to your org" },{ icon:"💬", title:"Q&A Included", desc:"All your questions answered" }].map(f => (
            React.createElement(Card, { key: f.title, style: { textAlign:"center", padding:18 },}
              , React.createElement('div', { style: { fontSize:24, marginBottom:8 },}, f.icon)
              , React.createElement('div', { style: { fontWeight:700, fontSize:13, color:C.dark },}, f.title)
              , React.createElement('div', { style: { fontSize:11, color:C.soft, marginTop:3 },}, f.desc)
            )
          ))
        )
      )
...
function Dashboard({ user, setUser, chatHistory, iepHistory, savedResources, waitlist, referrals, setActive }) {
  ...
  if (!user) return (
    React.createElement('div', { style: { paddingTop:80, minHeight:"100vh", background:C.cream, display:"flex", alignItems:"center", justifyContent:"center", padding:"80px 20px" },}
      , React.createElement(Card, { style: { maxWidth:400, width:"100%", padding:36 },}
        , React.createElement('div', { style: { textAlign:"center", marginBottom:24 },}
          , React.createElement('div', { style: { fontSize:36, marginBottom:8 },}, "🧩")
          , React.createElement('h2', { style: { fontFamily:serif, fontSize:24, fontWeight:900, color:C.dark, margin:"0 0 5px" },}, mode==="signin"?"Welcome back":"Join SpectraGuide")
          , React.createElement('p', { style: { color:C.mid, fontSize:13 },}, mode==="signin"?"Sign in to your account":"Create your free account")
        )
        , React.createElement('div', { style: { display:"flex", flexDirection:"column", gap:10 },}
          , mode==="signup" && React.createElement(Input, { value: form.name, onChange: e=>setForm({...form,name:e.target.value}), placeholder: "Your full name"  , style: { width:"100%", boxSizing:"border-box" },} )
          , React.createElement(Input, { value: form.email, onChange: e=>setForm({...form,email:e.target.value}), placeholder: "Email address" , type: "email", style: { width:"100%", boxSizing:"border-box" },} )
          , React.createElement(Input, { value: form.password, onChange: e=>setForm({...form,password:e.target.value}), placeholder: "Password", type: "password", style: { width:"100%", boxSizing:"border-box" },} )
          , err && React.createElement('div', { style: { fontSize:12, color:C.rose, fontWeight:600 },}, err)
          , React.createElement(Btn, { onClick: handleAuth, style: { width:"100%" },}, mode==="signin"?"Sign In":"Create Account")
        )
        , React.createElement('div', { style: { textAlign:"center", marginTop:16, fontSize:13, color:C.mid },}
          , mode==="signin"?"No account? ":"Already have one? "
          , React.createElement('button', { onClick: ()=>setMode(mode==="signin"?"signup":"signin"), style: { background:"none", border:"none", color:C.teal, fontWeight:700, cursor:"pointer", fontFamily:font, fontSize:13 },}, mode==="signin"?"Sign up free":"Sign in")
        )
      )
    )
  );
  ...
  return (
    React.createElement('div', { style: { paddingTop:80, minHeight:"100vh", background:C.cream, padding:"80px 20px 60px" },}
      , React.createElement('div', { style: { maxWidth:960, margin:"0 auto" },}
        , React.createElement(Card, { style: { marginBottom:22, background:`linear-gradient(135deg,${C.dark},#3D3860)` },}
          , React.createElement('div', { style: { display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:14 },}
            , React.createElement('div', { style: { display:"flex", alignItems:"center", gap:16 },}
              , React.createElement('div', { style: { width:56, height:56, borderRadius:14, background:`linear-gradient(135deg,${C.teal},${C.lavender})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:24, fontWeight:700, color:"white" },}, user.name[0].toUpperCase())
              , React.createElement('div', null
                , React.createElement('div', { style: { fontFamily:serif, fontSize:20, fontWeight:800, color:"white" },}, "Welcome, " , user.name, "!")
                , React.createElement('div', { style: { fontSize:12, color:"rgba(255,255,255,0.55)", marginTop:2 },}, user.email, " · "  , user.plan, " Plan · Joined "    , user.joined)
              )
            )
            , React.createElement('div', { style: { display:"flex", gap:8 },}
              , React.createElement(Btn, { variant: "gold", size: "sm", onClick: ()=>setActive("Pricing"),}, "Upgrade 💎" )
              , React.createElement(Btn, { variant: "danger", size: "sm", onClick: ()=>setUser(null),}, "Sign Out" )
            )
          )
        )

        , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr 1fr":"repeat(4,1fr)", gap:14, marginBottom:22 },}
          , [{ icon:"💬", label:"Chat Messages", value:chatHistory.filter(m=>m.role==="user").length, color:C.teal },{ icon:"📋", label:"IEPs Analyzed", value:iepHistory.length, color:C.lavender },{ icon:"❤️", label:"Saved Resources", value:savedResources.length, color:C.rose },{ icon:"👥", label:"Referrals", value:referrals, color:C.gold }].map(s => (
            React.createElement(Card, { key: s.label, style: { textAlign:"center" },}
              , React.createElement('div', { style: { fontSize:22, marginBottom:7 },}, s.icon)
              , React.createElement('div', { style: { fontFamily:serif, fontSize:26, fontWeight:900, color:s.color },}, s.value)
              , React.createElement('div', { style: { fontSize:11, color:C.soft, marginTop:3 },}, s.label)
            )
          ))
        )

        /* Referral Program */
        , React.createElement(Card, { style: { marginBottom:22, background:`linear-gradient(135deg,${C.teal}12,${C.lavender}12)` },}
          , React.createElement('div', { style: { display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 },}
            , React.createElement('div', null
              , React.createElement('div', { style: { fontWeight:800, fontSize:15, color:C.dark, marginBottom:4 },}, "🎁 Refer a Friend — Earn a Free Month"        )
              , React.createElement('div', { style: { fontSize:13, color:C.mid },}, "Share your link. For every friend who upgrades, you both get one free month of Family plan."                )
            )
            , React.createElement('div', { style: { display:"flex", gap:8, alignItems:"center" },}
              , React.createElement('div', { style: { background:"white", borderRadius:9, padding:"8px 14px", fontSize:12, fontWeight:700, color:C.teal, border:`1.5px solid ${C.teal}33` },}, refLink)
              , React.createElement(Btn, { size: "sm", onClick: ()=>_optionalChain([navigator, 'access', _39 => _39.clipboard, 'optionalAccess', _40 => _40.writeText, 'call', _41 => _41(`https://${refLink}`)]),}, "Copy")
            )
          )
        )

        , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr":"1fr 1fr", gap:18 },}
          , React.createElement(Card, null
            , React.createElement('div', { style: { display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 },}
              , React.createElement('div', { style: { fontWeight:800, fontSize:14, color:C.dark },}, "💬 Recent Chats"  )
              , React.createElement(Btn, { variant: "ghost", size: "sm", onClick: ()=>setActive("Chat"),}, "Open")
            )
            , chatHistory.filter(m=>m.role==="user").slice(-4).map((m,i) => (
              React.createElement('div', { key: i, style: { background:C.cream, borderRadius:8, padding:"8px 11px", marginBottom:7, fontSize:12, color:C.mid },}, "\"", m.content.slice(0,65), m.content.length>65?"…":"", "\"")
            ))
            , chatHistory.filter(m=>m.role==="user").length===0 && React.createElement('div', { style: { fontSize:12, color:C.soft, textAlign:"center", padding:"16px 0" },}, "No conversations yet. "   , React.createElement('button', { onClick: ()=>setActive("Chat"), style: { background:"none", border:"none", color:C.teal, fontWeight:700, cursor:"pointer", fontFamily:font, fontSize:12 },}, "Start chatting →"  ))
          )
          , React.createElement(Card, null
            , React.createElement('div', { style: { display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 },}
              , React.createElement('div', { style: { fontWeight:800, fontSize:14, color:C.dark },}, "📋 IEP History"  )
              , React.createElement(Btn, { variant: "ghost", size: "sm", onClick: ()=>setActive("IEP"),}, "Analyze New" )
            )
            , iepHistory.slice(0,4).map((h,i) => (
              React.createElement('div', { key: i, style: { background:C.cream, borderRadius:8, padding:"8px 11px", marginBottom:7 },}
                , React.createElement('div', { style: { display:"flex", justifyContent:"space-between" },}
                  , React.createElement('span', { style: { fontSize:12, fontWeight:600, color:C.dark },}, _optionalChain([h, 'access', _42 => _42.result, 'optionalAccess', _43 => _43.documentType]), " — "  , _optionalChain([h, 'access', _44 => _44.result, 'optionalAccess', _45 => _45.overallScore]), "/10")
                  , React.createElement('span', { style: { fontSize:10, color:C.soft },}, h.date)
                )
                , React.createElement('div', { style: { fontSize:11, color:C.soft, marginTop:2 },}, h.text)
              )
            ))
            , iepHistory.length===0 && React.createElement('div', { style: { fontSize:12, color:C.soft, textAlign:"center", padding:"16px 0" },}, "No analyses yet. "   , React.createElement('button', { onClick: ()=>setActive("IEP"), style: { background:"none", border:"none", color:C.teal, fontWeight:700, cursor:"pointer", fontFamily:font, fontSize:12 },}, "Analyze an IEP →"   ))
          )
          , React.createElement(Card, null
            , React.createElement('div', { style: { display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 },}
              , React.createElement('div', { style: { fontWeight:800, fontSize:14, color:C.dark },}, "❤️ Saved Resources"  )
              , React.createElement(Btn, { variant: "ghost", size: "sm", onClick: ()=>setActive("Resources"),}, "Find More" )
            )
            , savedResources.slice(0,4).map((r,i) => (
              React.createElement('div', { key: i, style: { display:"flex", justifyContent:"space-between", background:C.cream, borderRadius:8, padding:"8px 11px", marginBottom:7 },}
                , React.createElement('div', null, React.createElement('div', { style: { fontSize:12, fontWeight:600, color:C.dark },}, r.name), React.createElement('div', { style: { fontSize:10, color:C.soft },}, r.type))
                , React.createElement(Tag, { c: C.teal,}, r.scope)
              )
            ))
            , savedResources.length===0 && React.createElement('div', { style: { fontSize:12, color:C.soft, textAlign:"center", padding:"16px 0" },}, "No saved resources. "   , React.createElement('button', { onClick: ()=>setActive("Resources"), style: { background:"none", border:"none", color:C.teal, fontWeight:700, cursor:"pointer", fontFamily:font, fontSize:12 },}, "Find resources →"  ))
          )
          , React.createElement(Card, null
            , React.createElement('div', { style: { fontWeight:800, fontSize:14, color:C.dark, marginBottom:12 },}, "⚙️ Account" )
            , [{ label:"Name", value:user.name },{ label:"Email", value:user.email },{ label:"Plan", value:user.plan+" (Free)" },{ label:"Member Since", value:user.joined }].map(f => (
              React.createElement('div', { key: f.label, style: { display:"flex", justifyContent:"space-between", padding:"9px 12px", background:C.cream, borderRadius:8, marginBottom:7 },}
                , React.createElement('span', { style: { fontSize:12, color:C.soft },}, f.label)
                , React.createElement('span', { style: { fontSize:12, fontWeight:600, color:C.dark },}, f.value)
              )
            ))
          )
        )
...
function AdminDashboard({ waitlist, bookings, iepHistory, chatHistory, savedResources }) {
  ...
  const recentActivity = [
    ...
    ...iepHistory.slice(-2).map(h=>({ type:"iep", text:`IEP analyzed — Score ${_optionalChain([h, 'access', _46 => _46.result, 'optionalAccess', _47 => _47.overallScore])}/10`, time:h.date, color:C.peach })),
    ...
    React.createElement('div', { style: { paddingTop:80, minHeight:"100vh", background:C.cream, padding:"80px 20px 60px" },}
      , React.createElement('div', { style: { maxWidth:1050, margin:"0 auto" },}
        , React.createElement('div', { style: { display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24, flexWrap:"wrap", gap:12 },}
          , React.createElement('div', null
            , React.createElement(Pill, { color: C.rose,}, "ADMIN")
            , React.createElement('h1', { style: { fontFamily:serif, fontSize:mobile?24:30, fontWeight:900, color:C.dark, margin:"10px 0 0" },}, "SpectraGuide Command Center"  )
          )
          , React.createElement('div', { style: { fontSize:12, color:C.soft },}, "Last updated: "  , new Date().toLocaleTimeString())
        )
        ...
        , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr 1fr":"repeat(4,1fr)", gap:14, marginBottom:22 },}
          , [{ icon:"👥", label:"Total Users", value:totalUsers.toLocaleString(), change:"+12% this week", color:C.teal },{ icon:"📋", label:"IEPs Analyzed", value:totalIEPs.toLocaleString(), change:"+8% this week", color:C.lavender },{ icon:"💬", label:"Chat Messages", value:totalChats.toLocaleString(), change:"+22% this week", color:C.peach },{ icon:"📅", label:"Demos Booked", value:(bookings.length+47).toString(), change:`+${bookings.length} new`, color:C.gold }].map(s => (
            React.createElement(Card, { key: s.label,}
              ...
              , React.createElement('div', { style: { fontFamily:serif, fontSize:26, fontWeight:900, color:s.color },}, s.value)
              , React.createElement('div', { style: { fontSize:11, color:C.soft, marginTop:2 },}, s.label)
              , React.createElement('div', { style: { fontSize:10, color:C.teal, fontWeight:600, marginTop:4 },}, s.change)
            )
          ))
        )

        , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr":"1fr 1fr", gap:18, marginBottom:18 },}
          , React.createElement(Card, null
            , React.createElement('div', { style: { fontWeight:800, fontSize:14, color:C.dark, marginBottom:14 },}, "📈 Waitlist Signups"  )
            , waitlist.length===0 ? React.createElement('div', { style: { color:C.soft, fontSize:13 },}, "No signups yet — share your launch link!"       ) : (
              React.createElement('div', null
                , waitlist.slice(-6).reverse().map((w,i) => (
                  React.createElement('div', { key: i, style: { display:"flex", justifyContent:"space-between", padding:"8px 0", borderBottom:i<Math.min(waitlist.length,6)-1?`1px solid ${C.border}`:"none" },}
                    , React.createElement('span', { style: { fontSize:13, color:C.dark },}, w.email)
                    , React.createElement('span', { style: { fontSize:11, color:C.soft },}, w.date)
                  )
                ))
                , React.createElement('div', { style: { marginTop:12, fontSize:12, fontWeight:700, color:C.teal },}, "Total: " , (waitlist.length+50234).toLocaleString(), " signups" )
              )
            )
          )
          , React.createElement(Card, null
            , React.createElement('div', { style: { fontWeight:800, fontSize:14, color:C.dark, marginBottom:14 },}, "📅 Demo Requests"  )
            , bookings.length===0 ? React.createElement('div', { style: { color:C.soft, fontSize:13 },}, "No bookings yet."  ) : (
              bookings.slice(-5).reverse().map((b,i) => (
                React.createElement('div', { key: i, style: { padding:"8px 0", borderBottom:i<Math.min(bookings.length,5)-1?`1px solid ${C.border}`:"none" },}
                  , React.createElement('div', { style: { fontSize:13, fontWeight:600, color:C.dark },}, b.name)
                  , React.createElement('div', { style: { fontSize:11, color:C.soft },}, b.org||b.role, " · "  , b.date, " " , b.time)
                )
              ))
            )
          )
        ...
        , React.createElement(Card, null
          , React.createElement('div', { style: { fontWeight:800, fontSize:14, color:C.dark, marginBottom:14 },}, "⚡ Recent Activity"  )
          , recentActivity.length===0 ? React.createElement('div', { style: { color:C.soft, fontSize:13 },}, "No activity yet."  ) : (
            recentActivity.map((a,i) => (
              React.createElement('div', { key: i, style: { display:"flex", gap:10, padding:"9px 0", borderBottom:i<recentActivity.length-1?`1px solid ${C.border}`:"none" },}
                , React.createElement('div', { style: { width:8, height:8, borderRadius:"50%", background:a.color, marginTop:6, flexShrink:0 },} )
                , React.createElement('div', { style: { flex:1, fontSize:13, color:C.mid },}, a.text)
                , React.createElement('div', { style: { fontSize:11, color:C.soft, flexShrink:0 },}, a.time)
              )
            ))
          )
        )
...
function PartnerPage({ setActive }) {
  ...
  if (submitted) return (
    React.createElement('div', { style: { paddingTop:80, minHeight:"100vh", background:C.cream, display:"flex", alignItems:"center", justifyContent:"center", padding:"80px 20px" },}
      , React.createElement(Card, { style: { maxWidth:460, width:"100%", textAlign:"center", padding:44 },}
        , React.createElement('div', { style: { fontSize:44, marginBottom:14 },}, "🤝")
        , React.createElement('h2', { style: { fontFamily:serif, fontSize:24, fontWeight:900, color:C.dark, margin:"0 0 10px" },}, "Application Received!" )
        , React.createElement('p', { style: { color:C.mid, fontSize:14, lineHeight:1.7 },}, "Thank you, "  , React.createElement('strong', null, form.name), ". Our partnerships team will review your application and reach out within 3 business days."              )
      )
    )
  );
  ...
  return (
    React.createElement('div', { style: { paddingTop:80, minHeight:"100vh", background:C.cream, padding:"80px 20px 60px" },}
      , React.createElement('div', { style: { maxWidth:1000, margin:"0 auto" },}
        , React.createElement('div', { style: { textAlign:"center", marginBottom:44 },}
          , React.createElement(Pill, { color: C.gold,}, "PARTNERSHIPS")
          , React.createElement('h1', { style: { fontFamily:serif, fontSize:mobile?28:36, fontWeight:900, color:C.dark, margin:"14px 0 10px", letterSpacing:"-0.02em" },}, "Grow together with "
               , React.createElement(GradText, { a: C.teal, b: C.lavender,}, "SpectraGuide")
          )
          , React.createElement('p', { style: { color:C.mid, fontSize:16, maxWidth:520, margin:"0 auto" },}, "We partner with therapy providers, school districts, advocacy orgs, and technology companies who share our mission."               )
        )

        , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr":"repeat(3,1fr)", gap:20, marginBottom:52 },}
          , tiers.map(t => (
            React.createElement(Card, { key: t.name, style: { border:`2px solid ${t.color}33` },}
              , React.createElement('div', { style: { fontSize:30, marginBottom:10 },}, t.emoji)
              , React.createElement('h3', { style: { fontSize:17, fontWeight:800, color:C.dark, margin:"0 0 5px" },}, t.name)
              , React.createElement('div', { style: { marginBottom:18 },}
                , t.benefits.map((b,i) => React.createElement('div', { key: i, style: { display:"flex", gap:8, marginBottom:7 },}, React.createElement('span', { style: { color:t.color, fontWeight:700 },}, "✓"), React.createElement('span', { style: { fontSize:13, color:C.mid },}, b)))
              )
              , React.createElement(Btn, { variant: "ghost", size: "sm", style: { borderColor:t.color, color:t.color, width:"100%" }, onClick: ()=>_optionalChain([document, 'access', _48 => _48.getElementById, 'call', _49 => _49("partner-form"), 'optionalAccess', _50 => _50.scrollIntoView, 'call', _51 => _51({behavior:"smooth"})]),}, t.cta)
            )
          ))
        )

        , React.createElement(Card, { id: "partner-form",}
          , React.createElement('h2', { style: { fontFamily:serif, fontSize:22, fontWeight:800, color:C.dark, margin:"0 0 20px" },}, "📋 Partner Application"  )
          , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr":"1fr 1fr", gap:14, marginBottom:14 },}
            , [{ label:"Your Name", key:"name" },{ label:"Organization", key:"org" },{ label:"Email", key:"email", type:"email" }].map(f => (
              React.createElement('div', { key: f.key,}
                , React.createElement('div', { style: { fontSize:12, fontWeight:700, color:C.mid, marginBottom:5 },}, f.label)
                , React.createElement(Input, { value: form[f.key], onChange: e=>setForm({...form,[f.key]:e.target.value}), placeholder: "", type: f.type||"text", style: { width:"100%", boxSizing:"border-box" },} )
              )
            ))
            , React.createElement('div', null
              , React.createElement('div', { style: { fontSize:12, fontWeight:700, color:C.mid, marginBottom:5 },}, "Organization Type" )
              , React.createElement('select', { value: form.type, onChange: e=>setForm({...form,type:e.target.value}), style: { width:"100%", border:`1.5px solid ${C.border}`, borderRadius:10, padding:"10px 14px", fontSize:14, fontFamily:font, background:C.cream, color:C.dark, outline:"none" },}
          ...
          , React.createElement('div', { style: { marginBottom:18 },}
            , React.createElement('div', { style: { fontSize:12, fontWeight:700, color:C.mid, marginBottom:5 },}, "Tell us about your organization and partnership goals"       )
          ...
          , React.createElement(Btn, { onClick: ()=>{ if(form.name&&form.email) setSubmitted(true); }, disabled: !form.name||!form.email, style: { width:"100%" },}, "Submit Partnership Application 🤝"   )
        )
      )
    )
  );
...
function PressKit() {
  ...
  return (
    React.createElement('div', { style: { paddingTop:80, minHeight:"100vh", background:C.cream, padding:"80px 20px 60px" },}
      , React.createElement('div', { style: { maxWidth:960, margin:"0 auto" },}
        , React.createElement('div', { style: { textAlign:"center", marginBottom:44 },}
          , React.createElement(Pill, { color: C.mid,}, "PRESS KIT" )
          , React.createElement('h1', { style: { fontFamily:serif, fontSize:mobile?28:36, fontWeight:900, color:C.dark, margin:"14px 0 10px", letterSpacing:"-0.02em" },}, "SpectraGuide Media Resources"  )
          , React.createElement('p', { style: { color:C.mid, fontSize:15, maxWidth:480, margin:"0 auto" },}, "Everything journalists and media need to cover SpectraGuide."       )
        )

        /* Brand */
        , React.createElement(Card, { style: { marginBottom:22 },}
          , React.createElement('h2', { style: { fontFamily:serif, fontSize:18, fontWeight:800, color:C.dark, margin:"0 0 14px" },}, "🧩 Brand Assets"  )
          , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr":"1fr 1fr 1fr", gap:14 },}
            , [{ bg:C.dark, text:"white", label:"Dark Logo" },{ bg:C.cream, text:C.dark, label:"Light Logo" },{ bg:`linear-gradient(135deg,${C.teal},${C.lavender})`, text:"white", label:"Gradient Logo" }].map(v => (
              React.createElement('div', { key: v.label, style: { background:v.bg, borderRadius:12, padding:"28px 20px", textAlign:"center", border:`1px solid ${C.border}` },}
                , React.createElement('div', { style: { fontFamily:serif, fontSize:20, fontWeight:900, color:v.text },}, "Spectra", React.createElement('span', { style: { color:v.text==="white"?C.teal:C.teal },}, "Guide"))
                , React.createElement('div', { style: { fontSize:10, color:v.text==="white"?"rgba(255,255,255,0.5)":C.soft, marginTop:8 },}, v.label)
              )
            ))
          )
          , React.createElement('div', { style: { marginTop:16, display:"flex", gap:10, flexWrap:"wrap" },}
            , [["Primary Teal","#4BBFAD"],["Lavender","#B8A9E3"],["Dark","#2D2A3E"],["Gold","#F6C85F"],["Rose","#F4707A"]].map(([name,hex]) => (
              React.createElement('div', { key: hex, style: { display:"flex", alignItems:"center", gap:6 },}
                , React.createElement('div', { style: { width:20, height:20, borderRadius:5, background:hex, border:`1px solid ${C.border}` },} )
                , React.createElement('span', { style: { fontSize:11, color:C.mid },}, name, " " , React.createElement('span', { style: { color:C.soft },}, hex))
              )
            ))
          )
        )
        ...
        /* Stats */
        , React.createElement(Card, { style: { marginBottom:22 },}
          , React.createElement('h2', { style: { fontFamily:serif, fontSize:18, fontWeight:800, color:C.dark, margin:"0 0 16px" },}, "📊 Key Statistics"  )
          , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr 1fr":"repeat(3,1fr)", gap:12 },}
            , stats.map(s => (
              React.createElement('div', { key: s.n, style: { background:C.cream, borderRadius:10, padding:"14px 16px" },}
                , React.createElement('div', { style: { fontFamily:serif, fontSize:22, fontWeight:900, color:C.teal },}, s.n)
                , React.createElement('div', { style: { fontSize:11, color:C.mid, marginTop:2 },}, s.l)
              )
            ))
          )
        )
        ...
        /* Fast Facts */
        , React.createElement(Card, { style: { marginBottom:22 },}
          , React.createElement('h2', { style: { fontFamily:serif, fontSize:18, fontWeight:800, color:C.dark, margin:"0 0 14px" },}, "⚡ Fast Facts"  )
          , facts.map((f,i) => React.createElement('div', { key: i, style: { display:"flex", gap:9, marginBottom:9 },}, React.createElement('span', { style: { color:C.teal, fontWeight:700, flexShrink:0 },}, "→"), React.createElement('span', { style: { fontSize:13.5, color:C.mid, lineHeight:1.55 },}, f)))
        )

        /* Coverage */
        , React.createElement(Card, { style: { marginBottom:22 },}
          , React.createElement('h2', { style: { fontFamily:serif, fontSize:18, fontWeight:800, color:C.dark, margin:"0 0 14px" },}, "📰 Press Coverage"  )
          , coverage.map((c,i) => (
            React.createElement('div', { key: i, style: { padding:"11px 0", borderBottom:i<coverage.length-1?`1px solid ${C.border}`:"none" },}
              , React.createElement('div', { style: { display:"flex", gap:10, alignItems:"baseline", flexWrap:"wrap" },}
                , React.createElement('span', { style: { fontWeight:800, fontSize:13, color:C.teal },}, c.outlet)
                , React.createElement('span', { style: { fontSize:11, color:C.soft },}, c.date)
              )
              , React.createElement('div', { style: { fontSize:13.5, color:C.dark, marginTop:3, fontStyle:"italic" },}, "\"", c.headline, "\"")
            )
          ))
        )

        /* Contact */
        , React.createElement(Card, { style: { background:`linear-gradient(135deg,${C.teal}10,${C.lavender}10)`, textAlign:"center", padding:36 },}
          , React.createElement('div', { style: { fontSize:32, marginBottom:10 },}, "📬")
          , React.createElement('h2', { style: { fontFamily:serif, fontSize:20, fontWeight:800, color:C.dark, margin:"0 0 8px" },}, "Media Inquiries" )
          , React.createElement('p', { style: { color:C.mid, fontSize:14, marginBottom:16, lineHeight:1.7 },}, "For interviews, press releases, and media assets, contact our communications team."          )
          , React.createElement('div', { style: { display:"flex", gap:10, justifyContent:"center", flexWrap:"wrap" },}
            , React.createElement('div', { style: { background:"white", borderRadius:9, padding:"9px 18px", fontSize:13, fontWeight:700, color:C.teal, border:`1.5px solid ${C.teal}33` },}, "press@spectraguide.com")
            , React.createElement('div', { style: { background:"white", borderRadius:9, padding:"9px 18px", fontSize:13, fontWeight:700, color:C.mid, border:`1.5px solid ${C.border}` },}, "@SpectraGuide")
          )
        )
      )
    )
    ...
    React.createElement('div', { style: { paddingTop:80, minHeight:"100vh", background:C.cream, padding:"80px 20px 60px" },}
      , React.createElement('div', { style: { maxWidth:860, margin:"0 auto" },}
        ...
        , React.createElement('div', { style: { textAlign:"center", marginBottom:40 },}
          , React.createElement(Pill, { color: C.rose,}, "OUR STORY" )
          , React.createElement('h1', { style: { fontFamily:serif, fontSize:mobile?28:38, fontWeight:900, color:C.dark, margin:"16px 0 16px", letterSpacing:"-0.02em", lineHeight:1.15 },}, "Built from real life,"
               , React.createElement('br', null ), React.createElement(GradText, { a: C.teal, b: C.lavender,}, "real love, real purpose."   )
          )
        )

        , React.createElement(Card, { glow: true, style: { marginBottom:36 },}
          , React.createElement('div', { style: { display:"flex", flexDirection:mobile?"column":"row", gap:32, alignItems:"flex-start" },}
            ...
            , React.createElement('div', { style: { flexShrink:0, textAlign:"center" },}
              ...
              , React.createElement('div', { style: { fontWeight:800, fontSize:14, color:C.dark, marginTop:10 },}, "Tatyana Warren" )
              , React.createElement('div', { style: { fontSize:11, color:C.soft, marginTop:3 },}, "Founder & CEO"  )
              , React.createElement('div', { style: { fontSize:11, color:C.soft },}, "Kokomo, Indiana" )
              , React.createElement('div', { style: { fontSize:11, color:C.teal, marginTop:4, fontWeight:600 },}, "Mom of an autistic son 💙"     )
            )
            , React.createElement('div', { style: { flex:1 },}
              , React.createElement('div', { style: { fontSize:32, color:C.teal, fontFamily:serif, lineHeight:1, marginBottom:8 },}, "\"")
              , [
                ...
                React.createElement('p', { key: i, style: { fontSize:14, color:C.mid, lineHeight:1.85, margin:"0 0 14px" },}, para)
              ))
            )
          )
        )
        ...
        /* MISSION CARDS */
        , React.createElement('div', { style: { textAlign:"center", marginBottom:24 },}
          , React.createElement(Pill, { color: C.teal,}, "OUR MISSION" )
          , React.createElement('h2', { style: { fontFamily:serif, fontSize:mobile?22:28, fontWeight:800, color:C.dark, margin:"12px 0 6px" },}, "What drives everything we build"    )
        )
        , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr":"1fr 1fr", gap:18, marginBottom:44 },}
          , [
            ...
            React.createElement(Card, { key: f.t,}
              , React.createElement('div', { style: { fontSize:28, marginBottom:10 },}, f.e)
              , React.createElement('h3', { style: { fontSize:15, fontWeight:800, color:C.dark, margin:"0 0 7px" },}, f.t)
              , React.createElement('p', { style: { fontSize:13.5, color:C.mid, lineHeight:1.65, margin:0 },}, f.d)
            )
          ))
        )

        /* CTA */
        , React.createElement(Card, { style: { background:`linear-gradient(135deg,${C.teal}12,${C.lavender}12)`, textAlign:"center", padding:48 },}
          , React.createElement('div', { style: { fontSize:40, marginBottom:14 },}, "🤝")
          , React.createElement('h2', { style: { fontFamily:serif, fontSize:26, fontWeight:800, color:C.dark, margin:"0 0 10px" },}, "Partner with SpectraGuide"  )
          , React.createElement('p', { style: { color:C.mid, fontSize:14, maxWidth:440, margin:"0 auto 24px", lineHeight:1.75 },}, "We're partnering with school districts, therapy providers, advocacy organizations, and mission-aligned investors to reach every family who needs us."                  )
          , React.createElement('div', { style: { display:"flex", gap:10, justifyContent:"center", flexWrap:"wrap" },}
            , React.createElement(Btn, { onClick: ()=>setActive("Partner"),}, "🤝 Become a Partner"   )
            , React.createElement(Btn, { variant: "secondary", onClick: ()=>setActive("Booking"),}, "📅 Book a Demo"   )
          )
        )

        /* LEGAL LINKS */
        , React.createElement('div', { style: { textAlign:"center", marginTop:32, display:"flex", gap:20, justifyContent:"center", flexWrap:"wrap" },}
          , React.createElement('a', { href: "https://www.iubenda.com/privacy-policy/3224602", target: "_blank", rel: "noopener noreferrer" , style: { fontSize:12, color:C.soft, textDecoration:"none", borderBottom:`1px solid ${C.border}` },}, "Privacy Policy" )
          , React.createElement('a', { href: "https://www.iubenda.com/terms-and-conditions/3224602", target: "_blank", rel: "noopener noreferrer" , style: { fontSize:12, color:C.soft, textDecoration:"none", borderBottom:`1px solid ${C.border}` },}, "Terms of Service"  )
          , React.createElement('span', { style: { fontSize:12, color:C.soft },}, "© 2026 SpectraGuide · Made with 💙 in Kokomo, Indiana"         )
        )
...
function Footer({ setActive, t }) {
  const w = useWindowWidth(); const mobile = w<768;
  return (
    React.createElement('footer', { style: { background:C.dark, padding:"44px 24px 24px" },}
      , React.createElement('div', { style: { maxWidth:1050, margin:"0 auto" },}
        , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr 1fr":"2fr 1fr 1fr 1fr 1fr", gap:32, marginBottom:32 },}
          , React.createElement('div', null
            , React.createElement('div', { style: { display:"flex", alignItems:"center", gap:7, marginBottom:12 },}
              , React.createElement('div', { style: { width:28, height:28, borderRadius:8, background:`linear-gradient(135deg,${C.teal},${C.lavender})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:13 },}, "🧩")
              , React.createElement('span', { style: { fontFamily:serif, fontSize:17, fontWeight:900, color:"white" },}, "Spectra", React.createElement('span', { style: { color:C.teal },}, "Guide"))
            )
            , React.createElement('p', { style: { fontSize:12, color:"rgba(255,255,255,0.45)", lineHeight:1.7, maxWidth:220 },}, "Your AI-powered autism advocate. Empowering families, educators, and individuals every step of the journey."             )
          )
          , [{ title:"Platform", links:[["Chat","Chat"],["IEP Analyzer","IEP"],["Resources","Resources"],["Learning Hub","Blog"]] },
            ...
            React.createElement('div', { key: col.title,}
              , React.createElement('div', { style: { fontSize:10, fontWeight:700, color:"rgba(255,255,255,0.35)", letterSpacing:"0.1em", marginBottom:12, textTransform:"uppercase" },}, col.title)
              , col.links.map(([label,tab]) => (
                tab === "Privacy" 
                  ? React.createElement('a', { key: label, href: "https://www.iubenda.com/privacy-policy/3224602", target: "_blank", rel: "noopener noreferrer" , style: { fontSize:12, color:"rgba(255,255,255,0.5)", marginBottom:8, cursor:"pointer", display:"block", textDecoration:"none" },}, label)
                  : tab === "Terms"
                  ? React.createElement('a', { key: label, href: "https://www.iubenda.com/terms-and-conditions/3224602", target: "_blank", rel: "noopener noreferrer" , style: { fontSize:12, color:"rgba(255,255,255,0.5)", marginBottom:8, cursor:"pointer", display:"block", textDecoration:"none" },}, label)
                  : React.createElement('div', { key: label, onClick: ()=>setActive(tab), style: { fontSize:12, color:"rgba(255,255,255,0.5)", marginBottom:8, cursor:"pointer" },}, label)
              ))
            )
          ))
        )
        , React.createElement('div', { style: { borderTop:"1px solid rgba(255,255,255,0.08)", paddingTop:18, display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:10 },}
          , React.createElement('div', { style: { fontSize:11, color:"rgba(255,255,255,0.3)" },}, "© 2026 SpectraGuide. Made with 💙 for the autism community."         )
          , React.createElement('div', { style: { display:"flex", alignItems:"center", gap:7 },}
            , React.createElement('span', { style: { fontSize:10, color:"rgba(255,255,255,0.22)" },}, t.poweredBy)
            , React.createElement('span', { style: { background:`linear-gradient(135deg,${C.teal},${C.lavender})`, color:"white", fontSize:9, fontWeight:700, padding:"2px 9px", borderRadius:999 },}, "Claude Opus" )
          )
          , React.createElement('div', { style: { fontSize:11, color:"rgba(255,255,255,0.3)" },}, "Not a substitute for legal or medical advice."       )
        )
      )
    )
  );
...
function SEOStatePage({ state, setActive }) {
  ...
  return (
    React.createElement('div', { style: { paddingTop:80, minHeight:"100vh", background:C.cream, padding:"80px 20px 60px" },}
      , React.createElement('div', { style: { maxWidth:860, margin:"0 auto", textAlign:"center" },}
        , React.createElement(Pill, { color: C.teal,}, "RESOURCES")
        , React.createElement('h1', { style: { fontFamily:serif, fontSize:mobile?26:36, fontWeight:900, color:C.dark, margin:"14px 0 14px", letterSpacing:"-0.02em" },}, "Autism IEP Help & Resources in "      , state)
        , React.createElement('p', { style: { color:C.mid, fontSize:16, maxWidth:560, margin:"0 auto 32px", lineHeight:1.75 },}, "SpectraGuide helps families in "    , state, " navigate IEPs, find local autism resources, and understand their special education rights — powered by AI."                )
        , React.createElement('div', { style: { display:"grid", gridTemplateColumns:mobile?"1fr":"1fr 1fr 1fr", gap:16, marginBottom:36 },}
          , [{ icon:"📋", title:`${state} IEP Rights`, desc:"Understand your specific rights under {state} education code and federal IDEA.", btn:"Analyze Your IEP", tab:"IEP" },{ icon:"🌍", title:`Find Resources in ${state}`, desc:`Local ABA therapy, speech therapy, support groups, and legal advocates in ${state}.`, btn:"Search Resources", tab:"Resources" },{ icon:"💬", title:"Ask an AI Advocate", desc:`Get expert answers on ${state} special education law and autism supports.`, btn:"Start Chatting", tab:"Chat" }].map(c => (
            React.createElement(Card, { key: c.title, style: { textAlign:"left" },}
              , React.createElement('div', { style: { fontSize:28, marginBottom:10 },}, c.icon)
              , React.createElement('h3', { style: { fontSize:14, fontWeight:800, color:C.dark, margin:"0 0 7px" },}, c.title)
              , React.createElement('p', { style: { fontSize:12.5, color:C.mid, lineHeight:1.65, margin:"0 0 14px" },}, c.desc)
              , React.createElement(Btn, { size: "sm", onClick: ()=>setActive(c.tab),}, c.btn)
            )
          ))
        )
        , React.createElement(Btn, { size: "lg", onClick: ()=>setActive("Home"),}, "← Back to SpectraGuide"   )
      )
    )
  );
}
...
function App() {
  ...
  const resolvedActive = (active==="Admin" && !_optionalChain([user, 'optionalAccess', _52 => _52.isAdmin])) ? "Dashboard" : active;

  const sharedProps = { user, setUser, setActive, t, lang };

  const pages = {
    Home:      React.createElement(HomePage, { setActive: setActive, waitlist: waitlist, setWaitlist: setWaitlist, t: t,} ),
    Chat:      React.createElement(AdvocateChat, { ...sharedProps, chatHistory: chatHistory, setChatHistory: setChatHistory,} ),
    IEP:       React.createElement(IEPAnalyzer, { ...sharedProps, iepHistory: iepHistory, setIepHistory: setIepHistory,} ),
    Resources: React.createElement(ResourceFinder, { ...sharedProps, savedResources: savedRes, setSavedResources: setSavedRes,} ),
    Blog:      React.createElement(BlogHub, null ),
    Pricing:   React.createElement(PricingPage, { setActive: setActive,} ),
    Booking:   React.createElement(BookingPage, { bookings: bookings, setBookings: setBookings,} ),
    Dashboard: React.createElement(Dashboard, { ...sharedProps, chatHistory: chatHistory, iepHistory: iepHistory, savedResources: savedRes, waitlist: waitlist, referrals: referrals,} ),
    Admin:     React.createElement(AdminDashboard, { waitlist: waitlist, bookings: bookings, iepHistory: iepHistory, chatHistory: chatHistory, savedResources: savedRes,} ),
    Partner:   React.createElement(PartnerPage, { setActive: setActive,} ),
    Press:     React.createElement(PressKit, null ),
    About:     React.createElement(AboutPage, { setActive: setActive,} ),
  };

  return (
    React.createElement('div', { style: { fontFamily:font, background:C.cream, minHeight:"100vh" },}
      , React.createElement(Nav, { active: resolvedActive, setActive: setActive, user: user, setUser: setUser, lang: lang, setLang: setLang, t: t,} )
      , pages[resolvedActive] || pages.Home
      , resolvedActive !== "Chat" && React.createElement(Footer, { setActive: setActive, t: t,} )
    )
  );
}
